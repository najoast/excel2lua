...
func parseFields(fieldsName []string, fieldsDesc []string) []*field {
	...
	for i := 0; i < size; i++ {
		desc := strings.Split(strings.Replace(fieldsDesc[i],"||","|",-1), "|")
		f := &field{
			_index: i,
...
* @excel       : %s
* @sheet name  : %s
* @note        : This file was generated by excel2lua(go), DO NOT modify it!
* Copyright(C) %v ONEMT, All rights reserved
--]]`, sheetName, fileName, sheetName, currentTime.Year())
	...
	luaCode += "}\n"
	ioutil.WriteFile(outputPath + sheetName + ".lua", []byte(luaCode), 0644)
}
...
func main() {
	...
	inputFile := flag.String("input", "", "The xlsx file you want to convert.")
	flag.Parse()

	outpath := string(*outputPath)
	if outpath != "" {
	...
	for _, sheetName := range xlsx.GetSheetMap() {
		wg.Add(1)
		go processSheet(xlsx, *inputFile, sheetName, &wg, *isClient, outpath)
	}
	wg.Wait()
}
